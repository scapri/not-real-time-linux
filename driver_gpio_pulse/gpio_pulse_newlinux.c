...
static void set_gpio_altfunc(int gpio, int altfunc)
{
    ...
    printk("in set_gpio_altfunc, going to derenerence gpio_regs=%p\n", gpio_regs);
    old_value = gpio_regs->GPFSEL[reg_index];
                     ...
                     const char *buf, size_t len)
{
    ...
    return (r ? 0 : len);
#endif
gpio_direction_output(Gpio_pin_to_drive, 0);
return len;
}
...
static int __init gpio_pulse_init(void)
{
   ...
   //gpio_regs = (struct gpio_registers *) GPIO_REG_IO_VAL; 
